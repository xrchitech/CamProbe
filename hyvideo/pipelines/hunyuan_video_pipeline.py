...
from .my_utils import (
    depth_camera_grid,
    depth_camera_grid_perframe,
    depth_camera_grid_sequence,
    save_x0pred_grid_step,
    save_video_frame_grid,
    save_depth_grid,
    save_debug_stats,
)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if kwargs['warp'] == True:
            ...
            print('warp not allowed...')

        debug_warp = kwargs.get("debug_warp", False)
        debug_warp_dir = kwargs.get("debug_warp_dir", None)
        debug_frame_limit = kwargs.get("debug_frame_limit", 7)
        depth_input_source = kwargs.get("depth_input_source", "x0pred")
        ablation_mode = kwargs.get("ablation_mode", None)
        ablation_mode_name = "default_v" if ablation_mode is None else ablation_mode

        if ablation_mode_name not in ("default_v", "eps", "x0", "xt"):
            raise ValueError(
                f"ablation_mode should be None, 'default_v', 'eps', 'x0' or 'xt', but got {ablation_mode}"
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar, auto_offload_model(self.transformer, self.execution_device, enabled=self.enable_offloading):
            for i, t in enumerate(timesteps):
                ...
                if i >= kwargs['start_idx'] and i < kwargs['end_idx'] and kwargs['warp']== True:
                    # print(f'warping step {i}')
                    # x0pred before
                    sigma_t = self.scheduler.sigmas[i]
                    # print("scheduler sigma:", self.scheduler.sigmas[i].item())
                    # print("timestep:", t.item())
                    # print("t/T:", (t / self.scheduler.config.num_train_timesteps).item())
                    x0_pred_before = latents - sigma_t * noise_pred

                    # warp
                    lat_ch = noise_pred.shape[1]
                    lat_t = noise_pred.shape[2]
                    lat_h = noise_pred.shape[3]
                    lat_w = noise_pred.shape[4]

                    latents_decoded_xt = latents / self.vae.config.scaling_factor
                    latents_decoded_x0 = x0_pred_before / self.vae.config.scaling_factor
                    use_constant_depth = depth_input_source == "1"
                    video_frames = None
                    video_frames_xt = None
                    video_frames_x0 = None
                    if not use_constant_depth or debug_warp:
                        if depth_input_source == "latents":
                            latents_decoded = latents_decoded_xt
                        elif depth_input_source == "x0pred":
                            latents_decoded = latents_decoded_x0
                        elif depth_input_source != "1":
                            raise ValueError(f"depth_input_source should be 'latents', 'x0pred' or '1', but got {depth_input_source}")
                        ...
                        with torch.autocast(device_type="cuda", dtype=self.vae_dtype, enabled=self.vae_autocast_enabled), auto_offload_model(self.vae, self.execution_device, enabled=self.enable_offloading), self.vae.memory_efficient_context():
                            if not use_constant_depth:
                                video_frames = self.vae.decode(latents_decoded, return_dict=False, generator=generator)[0]
                                video_frames = (video_frames / 2 + 0.5).clamp(0, 1).cpu().float()

                            if debug_warp:
                                video_frames_xt = self.vae.decode(latents_decoded_xt, return_dict=False, generator=generator)[0]
                                video_frames_xt = (video_frames_xt / 2 + 0.5).clamp(0, 1).cpu().float()
                                video_frames_x0 = self.vae.decode(latents_decoded_x0, return_dict=False, generator=generator)[0]
                                video_frames_x0 = (video_frames_x0 / 2 + 0.5).clamp(0, 1).cpu().float()
                    ...
                    with torch.autocast(device_type="cuda", dtype=self.target_dtype, enabled=self.autocast_enabled):
                        ...
                        for f in range(lat_t):
                            if use_constant_depth:
                                depth_f = torch.ones(
                                    (latents.shape[0], 1, lat_h, lat_w),
                                    device=latents.device,
                                    dtype=latents.dtype,
                                )
                            else:
                                f_decode = f * self.vae.ffactor_temporal
                                depth_f = midas(video_frames[:, :, f_decode])   # [1,1,H,W]
                                depth_f = torch.nn.functional.interpolate(depth_f, size=(lat_h,lat_w), mode='bilinear',align_corners=True)
                            depth_list.append(depth_f)

                            if debug_warp:
                                f_decode = f * self.vae.ffactor_temporal
                                depth_xt = midas(video_frames_xt[:, :, f_decode])
                                depth_xt = torch.nn.functional.interpolate(depth_xt, size=(lat_h,lat_w), mode='bilinear', align_corners=True)
                                depth_list_xt.append(depth_xt)
                        ...
                        if debug_warp and debug_warp_dir is not None and get_rank() == 0:
                            ...
                            frame_indices = list(range(min(lat_t, debug_frame_limit)))

                            save_video_frame_grid(
                                video_frames_xt,
                                os.path.join(step_debug_dir, "decoded_xt.png"),
                                nrow=min(debug_frame_limit, lat_t),
                                frame_indices=frame_indices,
                            )
                            save_video_frame_grid(
                                video_frames_x0,
                                os.path.join(step_debug_dir, "decoded_x0pred.png"),
                                nrow=min(debug_frame_limit, lat_t),
                                frame_indices=frame_indices,
                            )
                            save_depth_grid(
                                depth_list_xt[:debug_frame_limit],
                                os.path.join(step_debug_dir, "depth_xt.png"),
                                nrow=min(debug_frame_limit, lat_t),
                            )
                            save_depth_grid(
                                depth_list_x0[:debug_frame_limit],
                                os.path.join(step_debug_dir, "depth_x0pred.png"),
                                nrow=min(debug_frame_limit, lat_t),
                            )

                            stats = {
                                "step": int(i),
                                "sigma_t": float(sigma_t.item() if torch.is_tensor(sigma_t) else sigma_t),
                                "decode_source_used_for_warp_depth": depth_input_source,
                                "xt_video_min": float(video_frames_xt.min().item()),
                                "xt_video_max": float(video_frames_xt.max().item()),
                                "xt_video_mean": float(video_frames_xt.mean().item()),
                                "x0_video_min": float(video_frames_x0.min().item()),
                                "x0_video_max": float(video_frames_x0.max().item()),
                                "x0_video_mean": float(video_frames_x0.mean().item()),
                                "xt_depth_min": float(torch.stack(depth_list_xt).min().item()),
                                "xt_depth_max": float(torch.stack(depth_list_xt).max().item()),
                                "xt_depth_median_f0": float(depth_list_xt[0].median().item()),
                                "x0_depth_min": float(torch.stack(depth_list_x0).min().item()),
                                "x0_depth_max": float(torch.stack(depth_list_x0).max().item()),
                                "x0_depth_median_f0": float(depth_list_x0[0].median().item()),
                            }
                            save_debug_stats(os.path.join(step_debug_dir, "decode_depth_stats.json"), stats)
                        ...
                        if kwargs['warp_mode'] == 'custom':
                            ...
                            t_cum = torch.zeros(1,3).to(self.device).to(x0_pred_before.dtype)
                            R_list = []
                            t_list = []
                            for f in range(lat_t):
                                R_list.append(R_cum)
                                ...
                                t_cum = R_delta @ t_cum.unsqueeze(-1)
                                t_cum = t_cum.squeeze(-1) + t_delta
                                R_cum = R_delta @ R_cum
                        else:
                        ...
                        for f in range(lat_t):
                            if depth_norm == 'raw':
                                grid = depth_camera_grid(
                                    depth=depth_list[f],    # or directly use first-frame depth depth[:, 0] is also ok
                                    R=R_list[f],
                                    t=t_list[f],
                                    H=lat_h,
                                    W=lat_w,
                                )
                            elif depth_norm == 'perframe':
                                grid = depth_camera_grid_perframe(
                                    depth=depth_list[f], 
                                    R=R_list[f],
                                    t=t_list[f],
                                    H=lat_h,
                            ...
                            elif depth_norm == 'sequence':
                                global_scale = depth_list[0].flatten(1).median(dim=1, keepdim=True).values[:, None]
                                grid = depth_camera_grid_sequence(
                                    depth=depth_list[f], 
                                    t=t_list[f],
                                    H=lat_h,
                                    W=lat_w,
                                ...
                                raise ValueError(f'depth norm shoule be raw, perframe or sequence, but got {depth_norm}')
                            if debug_warp:
                                grid_debug.append(grid.detach().float().cpu())
                            warp_source = x0_pred_before if ablation_mode_name != "xt" else latents
                        ...
                        warpeds = torch.stack(warpeds, dim=2)
                        x0pred_after = warpeds if ablation_mode_name != "xt" else None
                        xt_after = warpeds if ablation_mode_name == "xt" else None

                        if debug_warp and debug_warp_dir is not None and get_rank() == 0:
                            step_debug_dir = os.path.join(debug_warp_dir, f"step_{i:03d}")
                            grid_tensor = torch.stack(grid_debug, dim=0)
                            grid_x = grid_tensor[..., 0]
                            grid_y = grid_tensor[..., 1]
                            out_of_bound = ((grid_x < -1) | (grid_x > 1) | (grid_y < -1) | (grid_y > 1)).float()
                            ...
                            save_x0pred_grid_step(
                                ...
                                nrow=min(debug_frame_limit, lat_t),
                                frame_indices=list(range(min(lat_t, debug_frame_limit))),
                                name="x0pred_after" if ablation_mode_name != "xt" else "xt_after",
                            )
                            save_debug_stats(
                                os.path.join(step_debug_dir, "grid_stats.json"),
                                {
                                    "step": int(i),
                                    "grid_x_min": float(grid_x.min().item()),
                                    "grid_x_max": float(grid_x.max().item()),
                                    "grid_y_min": float(grid_y.min().item()),
                                    "grid_y_max": float(grid_y.max().item()),
                                    "out_of_bound_ratio": float(out_of_bound.mean().item()),
                                    "out_of_bound_ratio_f0": float(out_of_bound[0].mean().item()),
                                    "x0pred_before_min": float(x0_pred_before.min().item()),
                                    "x0pred_before_max": float(x0_pred_before.max().item()),
                                    "x0pred_after_min": float(warpeds.min().item()),
                                    "x0pred_after_max": float(warpeds.max().item()),
                                },
                            )

                    if ablation_mode_name == "x0":
                        noise_pred = (latents - x0pred_after) / sigma_t
                    elif ablation_mode_name == "eps":
                        noise_pred = (latents - (1 - sigma_t) * x0pred_after) / sigma_t
                    elif ablation_mode_name == "xt":
                        latents = xt_after
                        ...
                        # x0pred after -> xt
                        # x0pred_after = x0_pred_before
                        # 1 epsilon parameterization
                        epsilon = (latents - (1 - sigma_t) * x0_pred_before) / sigma_t
                        latent_warped = (1 - sigma_t) * x0pred_after + sigma_t * epsilon

                        # 1.5 random epsilon parameterization
                        # epsilon = torch.randn_like(latents, device=latents.device, dtype=latents.dtype)
                        # latent_warped = (1 - sigma_t) * x0pred_after + sigma_t * epsilon

                        # 2 velocity parameterization
                        # latent_warped = x0pred_after + sigma_t * noise_pred

                        # 2.2 new velocity parameterization
                        # epsilon = torch.randn_like(latents, device=latents.device, dtype=latents.dtype)
                        # new_noise_pred = epsilon - x0pred_after
                        # latent_warped = x0pred_after + sigma_t * new_noise_pred
                
                        # xt -> unet
                        latents = latent_warped
                        latents_concat = torch.concat([latents, cond_latents], dim=1)
                        latent_model_input = torch.cat([latents_concat] * 2) if self.do_classifier_free_guidance else latents_concat
                        latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
